...
so your observations from running this experiment on the grading system might be limited to an error message complaining about 
too much time taken.)
...
minPay = 0
lowBal = balance/12.0
highBal = (balance * (1+annualInterestRate/12.0)**12)/12.0
minPay = (lowBal + highBal)/2.0
newBal = balance
...
while(True):
    ...
    while month <= 12:
        newBal -= minPay
        interest = annualInterestRate/12.0 * newBal
        newBal += interest
        month += 1
    if(newBal <= 0 and newBal >= -0.01):
        break
    elif(newBal > 0):
            lowBal = minPay
    else:
