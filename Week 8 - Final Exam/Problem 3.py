...
"""
We want to write a procedure that converts an American number (between 0 and 
99), written as a string, into the equivalent Mandarin.
...
def convert_to_mandarin(us_num):
    ...
    if len(us_num) == 1 and int(us_num) < 10:
        return trans[us_num]

    elif int(us_num) < 19:
        return trans.get('10') + ' ' + trans[us_num[1]]

    elif int(us_num[1]) == '0':
        return trans[us_num[0]] + ' ' + trans.get('10')
    
    else:
        return trans[us_num[0]] + ' ' + trans.get('10') + ' ' + trans[us_num[1]]
